...
spatial = ovarian %>%
  filter(tissue_category == "Tumor", sample_id %in% ids_biv) %>%
  mutate(phenotype = case_when(phenotype_cd19 == "CD19+" ~ "immune1",
                                 phenotype_cd68 == "CD68+" ~ "immune2",
                                 TRUE ~ "other")) %>%
  dplyr::select(cell_id, sample_id, x, y, phenotype) 
...
ovarian_FDA = run_fcm(mxFDAobject, model_name = "fit_lfcm",
                      formula = survival_time ~ age + stage, event = "event",
                      metric = "biv k", r = "r", value = "fundiff",
                      afcm = FALSE)
```
...
```{r fig_vpData_survival_biv, fig.width = 10, fig.height=9}
es/pp/cindex/r50 + plot_layout(heights = c(2, 2, 1.5, 1))
```
