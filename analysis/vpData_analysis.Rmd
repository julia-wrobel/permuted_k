...
Proportion of subjects with significant clustering
...
dfp %>%
  filter( sample_id %in% unique(clinical$sample_id),
          ...
          !is.na(pvalue)) %>%
  group_by(method, r) %>%
  summarize(prop = sum(pvalue < 0.05)/n()) %>%
  ungroup() %>%
  ggplot(aes(r, prop, color = method, group = method)) +
  geom_line()
```
...
df_pval = dfp %>% 
  filter(#pvalue != -Inf,
         sample_id %in% unique(clinical$sample_id)) %>%
  mutate(r = round(r),
         pvalue = log(pvalue)
         ) %>%
  ...
  filter(!(stat == "var" & method == "kperm"),
         !(stat == "pvalue" & method == "kperm"),
         !(method == "kperm"),
         !(stat == "pvalue" & method == "k")) %>%
  mutate(r = ifelse(r == 101, 100, r),
         r = factor(r),
...
df_pval %>%
  filter(value < 200000, value > -400) %>%
  ggplot(aes(r, value, group = interaction(r, method), fill = method)) +
  geom_boxplot() +
  facet_wrap(~stat, scales = "free") +
  #geom_hline(data = filter(dfp, stat == "log(p-value)"), aes(yintercept = log(0.05)), 
