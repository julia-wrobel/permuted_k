...
Get parameter values for each scenario.
...
```{r}
df = df %>%
  select(r, iter, type, scenario, method, time, pvalue) %>%
  mutate(method = factor(method, levels = c("kperm", "kepd", "kepdThin"),
                         labels = c("perm", "epd", "epd thin"))) %>%
  mutate(lambda_n = params[scenario,]$n,
         lambda_m = params[scenario,]$m)
...
```{r}
df %>%
  ggplot(aes(pvalue, group = method, color = method)) +
  geom_histogram() +
  facet_wrap(~type)
```
