---
title: "simulation results, univariate K"
author: "Julia Wrobel"
...
times_df = df %>%
  ...
  mutate(method = factor(method,
                         ...
                         labels = c("K", "Kinhom","perm", "KAMP",
                                    "KAMP lite"))) %>%
  group_by(method, lambda_n, abundance) %>%
  summarize(lower = quantile(time, .25), 
            upper = quantile(time, .75),
            time = median(time)) %>%
  ungroup() 

df = df %>%
  filter((type %in% c("hom", "inhom") & r == 0.5) | (type %in% c("homClust", "inhomClust", "bothClust") & r == 1.25),
         lambda_n != 2000,
         abundance != "0.01") %>%
  ...
  filter(value < 25) %>%
    mutate(method = factor(method,
                         ...
                         labels = c("K", "Kinhom","perm", "KAMP",
                                    "KAMP lite")),
         type = factor(type, levels = c("hom", "inhom", "homClust", 
                                        "inhomClust", "bothClust"),
...
pBothClust = df %>%
  ...
  labs(x = "Proportion of immune cells",
       y = "Degree of clustering",
       title = "Alternative: background and immune clustered") +
  theme_minimal() +
  scale_fill_manual(values = my_colors) +
  geom_hline(yintercept = 0, linetype = 2, color = "gray") +
  #theme(legend.position = "none",
...
p_times = times_df %>%
  ...
  theme(legend.position = "top",
        legend.title = element_blank()) +
    geom_errorbar(
    aes(ymin = log(lower), ymax = log(upper)),
    width = 0.15
  ) +
