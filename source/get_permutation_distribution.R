# X: ppp dataset
# r: radius used for Ripley's K
# nperm: number of permutation
get_permutation_distribution = function(X, r) {
  #m = length(id)
  m = sum(X$marks == "immune")
  npts = npoints(X)
  npairs =  npts * (npts - 1)
  W = Window(X)
  areaW = area(W)
  ...
  rmax = diameter(W) + 1
  close = closepairs(X, rmax, what="all")
  DIJ = close$d
  ...
  D = matrix(as.numeric(DIJ.mat<=r), npts, npts)
  Wr = D*trans.mat
  ...
  results = list(
    khat = khat,
    kepd = mu_, # K expectation under permutation distributions\
    kvpd = var_k,
